...
def parse_interval_value(value: str) -> Union[Fraction, float]:
    ...
    try:
        ...
        ratio = Fraction(val, 1) if isinstance(val, int) else val
        if float(ratio) <= 1.0:
            raise argparse.ArgumentTypeError("L'intervallo deve essere > 1")
        return ratio
...
def write_tun_file(output_base: str, ratios: List[float], basekey: int,
    ...
    """Esporta un file .tun (AnaMark TUN)."""
    tun_path = f"{output_base}.tun"
    lines = [
        "[Tuning]",
        "FormatVersion=1",
        f"Name=Generated by {__program_name__} {__version__}",
        "",
        "[Exact Tuning]",
        f"; basekey={basekey} basefrequency={basefrequency:.6f}Hz"
    ]
    ...
    for n in range(128):
        if 0 <= basekey <= 127 and basekey <= n < basekey + len(ratios):
            idx = n - basekey
            freq = basefrequency * float(ratios[idx]) if 0 <= idx < len(ratios) else tet_freq(n - basekey)
        else:
